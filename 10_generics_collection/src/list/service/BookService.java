...
//		}
	 ...
	 * @param index
	 * @param newPrice
	 * @return "[책 제목] 가격이 [이전 가격] -> [수정된 가격]으로 수정되었습니다"
	 */
	...
	public boolean removeBookTitle(String title) {
		...
		for(BookDTO book : bookList) {
			...
			// 전달 받은 책의 제목이 list에 있을 경우
			if(book.getTitle().equals(title)) {
				return bookList.remove(book); // 삭제 성공 시 true 반환
				// -> bookkList에 저장된 객체(BookDTO)가 equals() 오버라이딩이 되어 있어야 사용 가능한 방법
			}
		}
		return false;
	}

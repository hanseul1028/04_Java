...
public class SetService {
	 ...
	 * - 중복 데이터(객체) 저장 x
	 * 	-> 중복된 객체가 들어오면 덮어쓰기
	 * 	-> *중요* equals(), hashCode() 오버라이딩 필수 (동등 비교를 하기 위해서)
	 * 
	 * Set 인터페이스를 구현한 구현체
	...
	/**
	 * Hash 단어가 포함된 컬렉션은 무조건 equals(), hashCode가 오버라이딩 되어있어야 한다
	 * 
...
//		Set<Person> personSet = new LinkedHashSet<Person>();
		...
		// 3. equals() 오버라이딩 x, hashCode() 오버라이딩 o -> 중복 제거 안됨
		// 4. equals() 오버라이딩 o, hashCode() 오버라이딩 o -> 중복 제거 됨!!
	}
	
	/* [Iterator<T> Set<T>.iterator()]
	 * - 해당 set에 저장된 요소를 순차접근 할 수 있는 반복자(Iterator)를
	 * 반환하는 메서드
	 * 
	 * [Iterator<E> 인터페이스]
	 * - 반복자 기능 인터페이스

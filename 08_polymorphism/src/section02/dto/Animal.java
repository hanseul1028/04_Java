 ...
 * - abstract 예약어 작성
 * 
 * - 추상 클래스는 보통 미완성부분(추상 메서드)을 가지고 있어
 *   객체 생성이 불가능하다
 * - 직접 객체 생성은 불가능 하지만 자식 클래스에서 상속 받아
 *	 미완성 부분(추상 메서드)을 정의해서 완성 시켜주면 자식 객체는 생성 가능
 * 	(오버라이딩 강제화, 동적 바인딩)
 * 
 * - 추상 클래스도 생성자는 필수로 존재 해야만 한다
 *   왜? 상속 받은 자식 내부에 부모 부분이 생성되어야 하기 떄문에
 *  
 * - 부모 부분에서 추상 메서드는 어떻게 구현되는가?
 *  -> 오버라이딩된 자식의 메서드로 연결됨
 *  
 *  - 추상 클래스 사용 이유
 * 	  클래스 간의 공통된 기능을 공유하면서도

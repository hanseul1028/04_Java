...
public class TryCatchService {
	...
	/**
	 * 예외처리 (try-catch) + 다형성 (업 캐스칭) + catch 작성 순서
	 * -  catch문은 하위 타입부터 작성
...
//		}
		...
		// -> 도달 할 수 없는 InputMismatchException catch 블록이다
		// == 절대로 실행되지 않는 코드
	...
	public void test5() {
		...
		int input = 0;
		do {
			System.out.println("--- 강제로 발생 시켜 던질 예외 선택 ---");
			...
			input = sc.nextInt(); // InputMismatchException
			 ...
			 switch(input) {
			 case 1 : throw new ArithmeticException(); /* break; (없어도 됨)*/
			 ...
			 case 0 : System.out.println("종료 선택");
				...
				System.out.println("예외 최상위 부모 Exception으로 나머지 모든 예외 잡아서 처리");
			}
		}while(input != 0);
	 ...
	 * 	 마지막에 무조건 실행 되어야 하는 코드를 작성하는 구문

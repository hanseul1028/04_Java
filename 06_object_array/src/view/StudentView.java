...
public class StudentView {
	...
	private void allStudent() {
		...
		// StudentService 객체에서 모든 학생을 저장한 객체 배열(students)을 얻어와
		// 화면에 출력
		...
		for(StudentDTO std : students) {
			...
			System.out.println(std.toString()); // std.toString()
...
//			System.out.println(result); 
	...
	private void selectIndex() {
		...
		System.out.println("조회 할 인덱스 입력 : ");
	 ...
	 * - 이름을 입력 받아
	 *	   반환형    			클래스명.메서드명(매개 변수)
	 * 	StudentDTO StudentService.selectName(String name) 호츌
	 * 
	 * - 학생 정보 반환 받아서 출력
	 * 	 단, 반환받은 학생이 없을 경우(null)
	...
	private void updateIndex() {
		...
		// 인덱스를 입력 받아 검사
		System.out.println("수정할 학생 인덱스 번호 입력 : ");
		int index = sc.nextInt();
	...
	private void selectScore() {
		System.out.println("\n---- 학생 1명 점수 조회(점수, 합계, 평균) ----\n");
			...
			System.out.println("점수를 조회할 학생 인덱스 번호 입력 : ");
			int index = sc.nextInt();
